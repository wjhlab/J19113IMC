...
plot_clustering_heatmap_wrapper(fcs=output$fcs,
                                ...
                                fileName = 'Clusteringheatmap_all_fcs_40.pdf'); dev.off()
...
#% CELLS
ggp2<-ggplot(ggdfd,aes(x=case,y=proportion,fill=case,shape=timepoint))+
...
ggp2<-ggplot(ggdfd,aes(x=timegroup,y=densities,fill=timegroup))+
  geom_boxplot(outlier.shape=NA, lwd=0.25, color="black")+
  geom_jitter(width=0.2, size=1, color="blue")+
  facet_wrap(~cluster,ncol=4,scales="free")+
  ylab("# of Cells / mm^2")+
  stat_compare_means(ref.group = "OnTx",
                     method = "t.test",
                     paired=T,
                     label = "p.format",
                     label.x = 1.4,
...
pdf('Densities_box_merged_stats+ontx.pdf',width=7,height=8)
ggp2
...
## Stacked bars
bp <- ggplot(ggdfdclean, aes(x = timegroup, y = densities, fill=cluster, order=cluster)) +
...
pdf('Line_Plot_merged_statspvalue1.pdf',width=9.5,height=9);lp_styled;
dev.off()
